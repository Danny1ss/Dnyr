...
d_nyr = Client(
    "cleaner",
    13251350,
    "7861256414:AAEd-KEwUmn9ueOGs0WpWWOxbt5_xLZxki4",
    bot_token=token,
    in_memory=True
)
DANNY = token.split(':')[0]
...
@d_nyr.on_message(filters.group & filters.media, group=1)
    ...
    while not await asyncio.sleep(1.7):
        try:
            for chat_id in chats_db:
                ...
                try:
                    await d_nyr.delete_messages(chat_id, msgs_ids)
